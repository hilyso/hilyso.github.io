...
### 1.4 配置 ```.SPEC```文件
  ...
  ``` bash
  .... # 前面代码省略
  --with-ssl-dir=/usr/openssl \  # 根据 rpm 安装 openssl 和 openssl-devel的路径决定的
  %if ! %{without_openssl}
        --without-openssl \
  %endif 
  .... # 后面代码省略
...
{% note warning %}
**注意:**
更新的前提是, openssl 和 openssl-devel 已经升级至 ```openssl-1.1.1w``` 版本
{% endnote %}
...
## 四、 附件
  ...
  - 该附件中的配置, 不会生成 ```/etc/pam.d/sshd```文件, 完成安装后, 请用```OpenSSH7.4p1```版本的文件替换 ```/etc/pam.d/sshd```
